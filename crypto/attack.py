...
class Attack(object):
    ...
    def benchmark(self, attack, algorithm='rsa'):
        if algorithm not in ['rsa', 'rsa sig' 'el gamal', 'dsa', 'des']:
            print("Invalid algorithm passed")
            return None

        message = 'h'

        for key_size in range(3, 15):
    ...
    def el_gamal_dec(self, cipher, pub_key={}, appr = "brute"):
        p = pub_key['p']
        alpha = pub_key['alpha']
        beta = pub_key['beta']
        a = 0
            ...
            if len(factors) > 1:
                for prime in factors[1:]:
                    ...
                    while (p-1) % (prime**i) == 0:
                        ...
                        x = (x + prime**(i-1)*1) if right == left else x
                        i += 1
